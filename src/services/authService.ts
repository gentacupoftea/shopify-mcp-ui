...
class AuthService {
  ...
  async getCurrentUser(): Promise<User> {
    ...
    throw new Error(
      response.error?.message || "ユーザー情報の取得に失敗しました",
    );
  }
  ...
  async resetPassword(email: string): Promise<void> {
    ...
    if (!response.success) {
      throw new Error(
        response.error?.message || "パスワードリセットに失敗しました",
      );
    }
  }
  ...
  async changePassword(
    ...
    if (!response.success) {
      throw new Error(
        response.error?.message || "パスワード変更に失敗しました",
      );
    }
  }
  ...
  async setupTwoFactor(): Promise<{ qrCode: string; secret: string }> {
    const response =
      await api.post<APIResponse<{ qrCode: string; secret: string }>>(
        "/auth/2fa/setup",
      );
